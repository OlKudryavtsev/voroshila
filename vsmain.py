...
def main():
    ...
    dp.add_handler(CommandHandler('sendmessage', sendmessage))
    dp.add_handler(CommandHandler('showgamevideo', showgamevideo))
    dp.add_handler(CommandHandler('sendvideo', sendvideo))
    #admin handlers
    dp.add_handler(CommandHandler('admin', admin))
    dp.add_handler(CommandHandler('adminkey', adminkey))
    dp.add_handler(CommandHandler('whoisready', whoisready, pass_args=True))
    dp.add_handler(CommandHandler('possiblegames', possiblegames))
    dp.add_handler(CommandHandler('showpossiblegames', showpossiblegames))
    dp.add_handler(CommandHandler('showscores', showscores))
    dp.add_handler(CallbackQueryHandler(button))
    dp.add_handler(CommandHandler("set", set, pass_args=True))
    dp.add_handler(CommandHandler("setvideo", setvideo, pass_args=True))

    dp.add_handler(MessageHandler(Filters.text, keyboardecho))
    dp.add_handler(MessageHandler(Filters.text, echo))
    dp.add_handler(MessageHandler(Filters.all & (~ Filters.text), echonotext))
